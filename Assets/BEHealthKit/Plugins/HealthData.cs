...
namespace BeliefEngine.HealthKit
{
	...
	public class DateInterval : System.Object
	{
		...
		public DateInterval(XmlNode node) {
			this.startDate = DateTimeBridge.DateFromString(node["startDate"].InnerText);
			this.endDate = DateTimeBridge.DateFromString(node["endDate"].InnerText);
			this.duration = Convert.ToDouble(node["duration"].InnerText);
		}
	}
	...
	public class QuantitySample : Sample {
		...
		public QuantitySample(XmlNode node) : base(node) {
			...
			if (node["minimumQuantity"] != null) this.minimumQuantity = Convert.ToDouble(node["minimumQuantity"].InnerText);
			if (node["maximumQuantity"] != null) this.maximumQuantity = Convert.ToDouble(node["maximumQuantity"].InnerText);
			if (node["averageQuantity"] != null) this.averageQuantity = Convert.ToDouble(node["averageQuantity"].InnerText);
			if (node["mostRecentQuantity"] != null) this.mostRecentQuantity = Convert.ToDouble(node["mostRecentQuantity"].InnerText);
			...
			if (node["sumQuantity"] != null) this.sumQuantity = Convert.ToDouble(node["sumQuantity"].InnerText);
		}

		/*! @brief convert to a reasonable string representation
		 */
		override public string ToString() {
			return string.Format("[{0}-{1} : {2}]", this.startDate.LocalDateTime, this.endDate.LocalDateTime, this.quantity);
		}
	}
	 ...
	 */
		...
		public HealthData(string xmlString) {
			...
			Debug.LogFormat("root name: {0}", this.RootName());
			...
			if (node != null && node.InnerText != null) {
				...
				if (Enum.TryParse<HKDataType>((string)node.InnerText, false, out dt)) {
					...
					if (Enum.TryParse<HKClinicalType>((string)node.InnerText, false, out ct)) {
						...
						Debug.LogError("no datatype or clinical type");
					}
				}
			} else {
