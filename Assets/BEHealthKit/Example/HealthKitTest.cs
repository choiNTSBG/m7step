...
public class HealthKitTest : MonoBehaviour {
	...
	public void AddObserver() {
		...
		this.healthStore.AddObserverQuery(HKDataType.HKWorkoutTypeIdentifier, delegate (List<Sample> samples, Error error) {
			string text = this.resultsLabel.text;
			foreach (Sample sample in samples) {
				WorkoutSample workoutSample = sample as WorkoutSample;
				if (workoutSample != null) {
					text = text + $"\n-{workoutSample.totalEnergyBurned}";
