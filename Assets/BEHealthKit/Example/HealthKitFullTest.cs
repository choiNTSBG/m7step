	...
	public void ReadData() {
		...
		if (!reading) {
			...
			try {
				...
				DateTimeOffset now = DateTimeOffset.UtcNow;
				//DateTimeOffset start = now.AddDays(-1);
				DateTimeOffset start = now.AddHours(-5);
				//DateTimeOffset aDayAgo = now.AddDays(-1);
				//DateTimeOffset aDayFromNow = now.AddDays(1);
				...
				if (dataType <= HKDataType.HKQuantityTypeIdentifierUVExposure) {
                    ...
                    //ReadQuantityData(dataType, aDayAgo, aDayFromNow);
                }
